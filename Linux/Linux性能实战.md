...
- S（Interrupt Sleep）：可中断的睡眠，表示进程因为等待某个事件而被系统挂起。当进程等待的事件发生时，它会被唤醒并进入R状态
...
中断是系统用来响应硬件设备请求的一种机制，会打断进程的正常调度和执行，然后调用内核中的终端处理程序来响应设备的请求。

Linux将中断处理过程分成了两个阶段，上半部分和下班部分
...
网卡接收到数据包后，会通过硬件中断的方式，通知内核有新的数据到了。这时，内核就应该调用中断处理程序来响应它。你可以自己先想一下，这种情况下的上半部和下半部分别负责什么工作呢？对上半部来说，既然是快速处理，其实就是要把网卡的数据读到内存中，然后更新一下硬件寄存器的状态（表示数据已经读好了），最后再发送一个软中断信号，通知下半部做进一步的处理。而下半部被软中断信号唤醒后，需要从内存中找到网络数据，再按照网络协议栈，对数据进行逐层解析和处理，直到把它送给应用程序。

> 上半部会打断CPU正在执行的任务，然后立即执行中断处理程序。而下班部以内核线程的方式执行，并且每个CPU都对应一个软终端内核线程，名字为`ksoftirqd/CPU_NUM`

软终端不止包括了硬件设备终端处理程序的下半部，一些内核自定义的事件也属于软中断，比如内核调度和RCU锁（Read-Copy Update）等

- /proc/softirqs:软中断的运行情况
- /proc/interrupts:硬中断的运行情况

![image-20200924180239419](Linux性能实战_images/image-20200924180239419.png)

其实对于软中断，使用top查看cpu列si使用情况，之后使用`watch -d cat /proc/softirqs`根据软中断变化率再去查找对应的问题
